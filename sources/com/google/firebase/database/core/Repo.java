    ...
    public Task<DataSnapshot> getValue(final Query query) {
        ...
        scheduleNow(new Runnable() {
            public void run() {
                ...
                Repo.this.connection.get(query.getPath().asList(), query.getSpec().getParams().getWireProtocolParams()).addOnCompleteListener(new OnCompleteListener<Object>() {
                    public void onComplete(Task<Object> task) {
                        if (!task.isSuccessful()) {
                            LogWrapper access$100 = Repo.this.operationLogger;
                            access$100.info("get for query " + query.getPath() + " falling back to cache after error: " + task.getException().getMessage());
                            Node cached = Repo.this.serverSyncTree.calcCompleteEventCache(query.getPath(), new ArrayList());
                            if (cached.isEmpty()) {
                                source.setException(task.getException());
                            } else {
                                source.setResult(InternalHelpers.createDataSnapshot(query.getRef(), IndexedNode.from(cached, query.getSpec().getIndex())));
                            }
                        } else {
        ...
        return source.getTask().addOnCompleteListener(new OnCompleteListener<DataSnapshot>() {
